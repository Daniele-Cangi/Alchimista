...
const fileLabel = value => esc(value).replaceAll("_", "_<wbr>");
const route = () => {
...
function piiTokens(types, limit=6) {
  const visible=(types||[]).slice(0,limit), remaining=Math.max(0,(types||[]).length-visible.length);
  return `${visible.map(type=>`<span>${esc(type)}</span>`).join("")}${remaining?`<span>+${remaining}</span>`:""}`;
}
function documentCards(documents, limit=100, withActions=false) {
  return documents.slice(0,limit).map((d,index) => {
    const fileType=(d.mime_type || "DOC").split("/").pop().slice(0,3).toUpperCase();
    const dossierNumber=String(documents.length-index).padStart(2,"0");
    return `<article class="document-card" data-doc="${esc(d.doc_id)}" role="link" tabindex="0" aria-label="Apri ${esc(d.name)}">
      <div class="doc-card-file"><span class="file-icon">${esc(fileType)}</span><small>Fascicolo ${dossierNumber}</small></div>
      <div class="doc-card-body"><div class="doc-card-heading"><div><span class="document-id">${esc(shortId(d.doc_id))}</span><h3>${fileLabel(d.name)}</h3></div>${statusBadge(d.status)}</div>
      <div class="doc-card-facts"><span><small>Passaggi</small><strong>${d.chunks || 0}</strong></span><span><small>PII rilevate</small><strong>${d.pii_detected || 0}</strong></span><span><small>Policy</small><strong>${esc(d.privacy_policy || "off")}</strong></span></div>
      <div class="doc-card-foot"><span class="privacy-trace"><i></i> Evidenza privacy registrata</span><time>${formatDate(d.updated_at)}</time></div></div>
      ${withActions?`<button class="document-remove" type="button" data-delete-doc="${esc(d.doc_id)}" aria-label="Rimuovi ${esc(d.name)}"><span aria-hidden="true">×</span><small>Rimuovi</small></button>`:""}</article>`;
  }).join("");
}
function bindDocRows() {
  document.querySelectorAll("[data-doc]").forEach(row => {
    const openDocument=event=>{if(event.target.closest("[data-delete-doc]"))return;navigate(`/documents/${encodeURIComponent(row.dataset.doc)}`)};
    row.addEventListener("click",openDocument);
    row.addEventListener("keydown",event=>{if((event.key==="Enter"||event.key===" ")&&!event.target.closest("[data-delete-doc]")){event.preventDefault();openDocument(event)}});
  });
...
async function documents() {
  ...
  app.innerHTML = `<div class="documents-stage"><section class="card upload-card"><div class="panel-number"><span>01</span> Acquisizione</div><div class="section-head"><div><p class="eyebrow">Ingresso controllato</p><h2>Porta un documento nel workspace</h2><p>PDF, testo o immagine · massimo ${formatBytes(state.uploadLimit)}</p></div></div>
    <label class="dropzone" id="dropzone"><input type="file" id="file"><span class="dropzone-sigil" aria-hidden="true">＋</span><strong>Trascina qui il documento</strong><p>Resta nel tuo stack locale e viene protetto secondo la policy attiva.</p><span class="button secondary">Scegli file</span></label><div id="upload-state"></div></section>
    <section class="card library-manifest"><div class="panel-number"><span>02</span> Custodia</div><span class="manifest-seal" aria-hidden="true">✦</span><p class="eyebrow">Libreria persistente</p><h2>Una memoria privata,<br>con una storia verificabile.</h2><p>I documenti rimangono disponibili dopo i riavvii. Ogni elaborazione conserva la policy e il detector realmente applicati.</p><div class="assurance-list"><span><i>✓</i> Storage locale</span><span><i>✓</i> Evidenze apribili</span><span><i>✓</i> Audit storico</span></div><div class="note">Cambiare policy non riscrive la storia: vale per nuove elaborazioni e reprocess espliciti.</div></section></div>
    <section class="library-section"><div class="library-heading"><div><p class="eyebrow">Archivio Mercury</p><h2>Tutti i documenti</h2><p>${docs.total || 0} elementi nel workspace <strong>${esc(state.workspace)}</strong></p></div><div class="library-legend"><span><i class="legend-dot ready"></i> Pronto</span><span><i class="legend-dot privacy"></i> Privacy registrata</span></div></div>
    ${(docs.documents || []).length ? `<div class="document-grid">${documentCards(docs.documents,100,true)}</div>` : empty("Libreria vuota", "Carica un file dal riquadro qui sopra.")}</section>`;
  bindDocRows();
...
async function documentDetail(id) {
  ...
  const fileType=(d.mime_type || "DOC").split("/").pop().slice(0,3).toUpperCase();
  app.innerHTML = `<section class="document-masthead"><div class="document-masthead-top"><a href="/documents" class="back-link">← Libreria documenti</a>${statusBadge(d.status)}</div><div class="document-identity"><span class="document-seal">${esc(fileType)}</span><div><p class="eyebrow">Fascicolo verificabile</p><h2>${fileLabel(d.name)}</h2><p>${esc(shortId(d.doc_id))} · ${formatBytes(d.size_bytes)} · acquisito ${formatDate(d.created_at)}</p></div><div class="document-actions"><a class="button secondary" href="/privacy">Rivedi privacy</a><a class="button" href="/ask?doc=${encodeURIComponent(d.doc_id)}">Chiedi al documento ✦</a></div></div></section>
    <section class="stats atelier-stats"><div class="card stat"><small>Passaggi</small><strong>${d.chunks}</strong><em>evidenze ricercabili</em></div><div class="card stat stat-pii"><small>PII rilevate</small><strong>${d.pii_detected}</strong><div class="pii-tokens">${piiTokens(d.pii_types)}</div></div><div class="card stat stat-policy"><small>Policy applicata</small><strong>${esc(d.privacy_policy)}</strong><em>${esc(d.privacy_detector || "nessun detector")}${d.privacy_engine_version?` · ${esc(d.privacy_engine_version)}`:""}${d.privacy_engine_source_revision?` · rev ${esc(d.privacy_engine_source_revision.slice(0,8))}`:""}</em></div><div class="card stat"><small>Decisioni</small><strong>${d.decisions_referencing}</strong><em>riferimenti audit</em></div></section>
    <section class="card evidence-ledger"><div class="section-head"><div><p class="eyebrow">Registro delle fonti</p><h2>Evidenze indicizzate</h2><p>Ogni estratto è apribile e conserva il riferimento al documento.</p></div><span class="ledger-count">${(d.evidence||[]).length} passaggi</span></div>
    ${(d.evidence||[]).length ? `<div class="citations evidence-list">${d.evidence.map(e=>`<button class="citation evidence-card" data-evidence="${esc(e.chunk_id)}"><span class="evidence-number">${String(e.chunk_index+1).padStart(2,"0")}</span><span class="evidence-copy"><strong>Passaggio ${e.chunk_index+1}</strong><small>${e.token_count} token · ${esc(shortId(e.chunk_id))}</small><p>${esc(e.preview.slice(0,300))}</p><span class="evidence-open">Apri evidenza →</span></span></button>`).join("")}</div>` : empty("Nessuna evidenza", "L’elaborazione potrebbe essere ancora in corso.")}</section>
...
async function ask() {
  ...
  const prompts=["Qual è il budget del Progetto Mercury e chi ne è responsabile?","Quale società svolge la revisione dei dati?","Confronta i tempi di conservazione previsti."];
  app.innerHTML = `<div class="ask-layout"><div class="stack"><section class="card ask-box"><div class="ask-orbit" aria-hidden="true">✦</div><p class="eyebrow">Domanda al tuo archivio</p><h2>Trasforma i documenti<br>in una risposta verificabile.</h2><p class="ask-intro">Formula una domanda naturale. Alchimista risponde soltanto con evidenze apribili dal tuo workspace.</p><textarea id="question" placeholder="Che cosa vuoi verificare?"></textarea><div class="prompt-suggestions">${prompts.map((prompt,index)=>`<button type="button" data-prompt="${index}">${esc(prompt)}</button>`).join("")}</div><div class="ask-submit-row"><span><i>✓</i> Fonti citabili · audit automatico · nessun dato fuori dal runtime</span><button class="button" id="ask-submit">Cerca nelle fonti <b>✦</b></button></div></section><section class="card answer-card" id="answer-card" hidden></section></div>
    <aside class="card search-scope"><div class="scope-head"><div><p class="eyebrow">Perimetro della ricerca</p><h3>Documenti da consultare</h3></div><span id="selection-count">${selectedFromQuery?"1 selezionato":"0 selezionati"}</span></div><p>Se non selezioni nulla, Alchimista cerca nell’intero archivio.</p><div class="doc-picker">${(docs.documents||[]).map(d=>`<label class="check-row"><input type="checkbox" value="${esc(d.doc_id)}" ${selectedFromQuery===d.doc_id?"checked":""}><span class="scope-file">PDF</span><span><strong>${fileLabel(d.name)}</strong><small class="subtle">${d.chunks} passaggi · ${d.pii_detected||0} PII</small></span></label>`).join("") || empty("Nessun documento", "Carica documenti prima di porre una domanda.")}</div><details class="recovery-options"><summary>Opzioni di recupero</summary><div class="field"><label for="top-k">Numero massimo di evidenze</label><input id="top-k" type="number" min="1" max="20" value="5"></div></details></aside></div>`;
  document.querySelectorAll("[data-prompt]").forEach(button=>button.addEventListener("click",()=>{document.querySelector("#question").value=prompts[Number(button.dataset.prompt)];document.querySelector("#question").focus()}));
  const updateSelection=()=>{const count=document.querySelectorAll('.doc-picker input:checked').length;document.querySelector('#selection-count').textContent=`${count} selezionat${count===1?'o':'i'}`};
  document.querySelectorAll('.doc-picker input').forEach(input=>input.addEventListener('change',updateSelection));
...
async function runAsk() {
  ...
  try { const result=await json("/api/v1/query",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant:state.workspace,query:question,k:Number(document.querySelector("#top-k").value)||5,doc_ids:docIds.length?docIds:null})});
    card.innerHTML = result.answer ? `<div class="answer-heading"><div><p class="eyebrow">Risposta basata sulle evidenze</p><h2>Risultato</h2></div><span class="score-seal"><small>Confidenza</small>${Number(result.score||0).toFixed(3)}</span></div><div class="answer">${esc(result.answer)}</div><div class="sources-heading"><h3>Fonti consultate</h3><span>${(result.citations||[]).length} citazioni apribili</span></div><div class="citations">${(result.citations||[]).map((c,i)=>`<button class="citation result-citation" data-doc-id="${esc(c.doc_id)}" data-chunk-id="${esc(c.chunk_id)}"><span class="citation-index">${String(i+1).padStart(2,"0")}</span><span><strong>${esc(c.document_name||shortId(c.doc_id))}</strong><small>Passaggio ${Number(c.chunk_index??0)+1}</small><p>${esc(c.preview||"")}</p><em>Apri la fonte →</em></span></button>`).join("")}</div>` : empty("Nessuna evidenza", "Prova a riformulare la domanda o amplia l’ambito.");
...
async function showEvidence(docId,chunkId){try{const e=await json(`/api/v1/documents/${encodeURIComponent(docId)}/evidence/${encodeURIComponent(chunkId)}?workspace=${encodeURIComponent(state.workspace)}`);document.querySelector("#evidence-content").innerHTML=`<p class="eyebrow">Evidenza verificabile</p><div class="modal-document-title"><span>${String(e.chunk_index+1).padStart(2,"0")}</span><div><h2>${esc(e.document_name)}</h2><p class="subtle">Passaggio ${e.chunk_index+1} · ${e.token_count} token</p></div></div><div class="evidence-text">${esc(e.preview)}</div><p><a class="button secondary" href="/documents/${encodeURIComponent(docId)}">Apri documento completo</a></p>`;document.querySelector("#evidence-modal").showModal();}catch(error){toast(error.message,true)}}
...
function modelCard(m){const busy=["DOWNLOADING","LOADING"].includes(m.state),appVersion=m.upstream_app_version||"2.0.0";return `<div class="model-state"><span class="model-orb">✦</span><span><strong>Rizzo PII Full 0.3B</strong><small>motore ${esc(appVersion)} · modello ${esc(m.revision||"v1.5.0")} · ${statusBadge(m.state||"NOT_INSTALLED")}</small></span></div>${busy?`<div class="progress-track indeterminate"><span></span></div><p>${esc((m.phase||"operazione in corso").replaceAll("_"," "))}</p>`:""}${m.error?`<div class="note">${esc(m.error)}</div>`:""}<div class="hero-actions">${!m.installed?'<button class="button" data-model="install">Installa modello</button>':''}${m.installed&&!m.loaded?'<button class="button" data-model="load">Carica in memoria</button>':''}${m.loaded?'<button class="button secondary" data-model="unload">Scarica dalla memoria</button>':''}</div><p class="subtle">La versione del motore e quella del modello sono indipendenti. I pesi restano sul volume locale anche quando il modello è scaricato dalla RAM.</p>`}
